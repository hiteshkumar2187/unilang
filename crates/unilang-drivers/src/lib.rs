...
impl DriverRegistry {
    ...
    /// Iterate over all registered drivers as trait objects.
    pub fn drivers(&self) -> &[Box<dyn UniLangDriver>] {
        &self.drivers
    }
}
...
// ── Community drivers (auto-discovered by build.rs) ──────────────────────────
pub mod community;

// Generated by build.rs: register_community_drivers + community_exported_functions
include!(concat!(env!("OUT_DIR"), "/community_drivers.rs"));
...
pub fn default_registry() -> DriverRegistry {
    ...
    // Auto-register any community drivers discovered at build time.
    register_community_drivers(&mut r);
